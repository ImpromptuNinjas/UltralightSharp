...
public static class Utilities {
  ...
  public static string LoadEmbeddedUtf8String(string resName) {
    using var stream = typeof(Utilities).Assembly.GetManifestResourceStream(resName);
    if (stream == null) throw new FileNotFoundException($"Missing embedded resource: {resName}");

    return LoadString(stream);
  }

  public static string LoadString(Stream stream) {
    using var sr = new StreamReader(stream, Encoding.UTF8, false, 16, true);
    return sr.ReadToEnd();
  }
